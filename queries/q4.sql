...
-- The percentage change in sessions from last quarter
with cte as
...
select *
,lag(session_count,1) over(order by first_quarter_date) as prev_session_count
,(session_count-(lag(session_count,1) over(order by first_quarter_date)))*100.0/(lag(session_count,1) over(order by first_quarter_date))
from cte;
